...
st.set_page_config(
    page_title="Previsão INDE",
    layout="wide",
    initial_sidebar_state="expanded",
)

df_ts_long = pd.read_csv('PM_modelo.csv', sep=';')

def obter_dados_e_previsao(id_aluno):
    df_aluno = df_ts_long[df_ts_long['ID'] == id_aluno].sort_values(by='Ano')
    if df_aluno.empty:
        return None, None, None

    serie_temporal = df_aluno['INDE'].values
    anos = df_aluno['Ano'].values
    modelo = auto_arima(serie_temporal, seasonal=False, stepwise=True, suppress_warnings=True, error_action="ignore", trace=False)
    predicao = modelo.predict(n_periods=1)[0]
    return serie_temporal, anos, predicao


def plotar_grafico_inde(serie_temporal, anos, predicao, id_aluno):
    plt.figure(figsize=(10, 5))

    anos_predicao = list(anos) + [max(anos) + 1]  
    valores_com_predicao = list(serie_temporal) + [predicao]

    plt.plot(anos, serie_temporal, marker='o', linestyle='-', label='INDE Real', color='b')

   
    for i, txt in enumerate(serie_temporal):
        plt.annotate(f"{txt:.2f}", (anos[i], serie_temporal[i]), textcoords="offset points", xytext=(0,10), ha='center')

    plt.plot(anos_predicao[-1], predicao, marker='o', color='r', label='INDE Previsto')

   
    plt.annotate(f"{predicao:.2f}", (anos_predicao[-1], predicao), textcoords="offset points", xytext=(0,10), ha='center')

 
    plt.plot([anos[-1], anos_predicao[-1]], [serie_temporal[-1], predicao], linestyle='--', color='r')

    plt.title(f'Histórico do INDE - Aluno {id_aluno}',  loc='left')
    plt.xlabel('Ano')
    plt.ylabel('INDE')
    
    valor_maximo = max(max(serie_temporal), predicao)
    plt.ylim(top=valor_maximo + valor_maximo * 0.05) 

    plt.xticks(range(min(anos), max(anos_predicao) + 1, 1))
    plt.legend()
    st.pyplot(plt)
...
def pagina_previsao():
    ...
    if id_aluno:
        try:
            ...
            serie_temporal, anos, predicao = obter_dados_e_previsao(id_aluno_int)
            if serie_temporal is not None:
                st.subheader(f'O INDE previsto para o aluno {id_aluno_int} é: {predicao:.2f}')
                st.markdown("##")
                plotar_grafico_inde(serie_temporal, anos, predicao, id_aluno_int)
            else:
